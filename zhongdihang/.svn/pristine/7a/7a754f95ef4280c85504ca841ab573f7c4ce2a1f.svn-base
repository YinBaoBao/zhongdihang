...
(function() {
	...
	var $WebUpload = function(pictureId) {
		...
		this.uploadUrl = Bank.route(Bank.api.file_up);
		this.fileSizeLimit = 100 * 1024 * 1024;
		this.picWidth = 800;
		this.picHeight = 800;
        this.uploadBarId = null;
		this.defaultimg="/img/default-house.jpg";
	};
	...
	$WebUpload.prototype = {
		...
		bindEvent : function(bindedObj) {
			...
			bindedObj.on('uploadBeforeSend', function(obj, data, headers) {
				$.extend(headers, {
					"token": Bank.getHeader()
				});
			});

			// 文件上传过程中创建进度条实时显示。
			bindedObj.on('uploadProgress', function(file, percentage) {
				$("#"+me.uploadBarId).parent().parent().show();
