...
public class RoleController {
    ...
    public ResultModel menuAssign(Long roleId,String menuIds){
        ...
        Long[] roleIdLong=new Long[menuIdsStr.length];
        for (int i=0;i<menuIdsStr.length;i++){
            roleIdLong[i]= Long.parseLong(menuIdsStr[i]);
        }
        roleService.roleMenu(roleId,roleIdLong);
        return ResultModel.ok();
    }
}
