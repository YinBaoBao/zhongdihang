...
public class GuarantyController {
    ...
    public ResultModel addGuaranty(@RequestBody GuarantyForm guarantyForm,Long fileId){
        ...
        guaranty.setDocument(guarantyForm.getDocument());
//        guaranty.setDispatchNo(guarantyForm.getDispatchNo());
        ...
        guaranty.setHandlerTelephone(guarantyForm.getHandlerTelephone());
    ...
    public ResultModel updateGuaranty(@RequestParam("id") Long id, @RequestParam("guarantyType") String guarantyType,@RequestParam("document") String document,
                                      @RequestParam("clientName") String clientName,@RequestParam("clientTelephone") String clientTelephone,
                                      @RequestParam("handlerName") String handlerName,@RequestParam("handlerTelephone") String handlerTelephone,
                                        @RequestParam Long fileId){
        File file = fileService.findFile(fileId);
        Guaranty guaranty = guarantyService.getGuaranty(id);
        if(file!=null){
            guaranty.setFile(file);
        }else {
            return ResultModel.error(ResultStatus.FAILURE,"无法找到抵押物信息");
        }
        guaranty.setGuarantyType(guarantyType);
        guaranty.setDocument(document);
        guaranty.setClientName(clientName);
        guaranty.setUpdateTime(new Date());
        guaranty.setClientTelephone(clientTelephone);
        guaranty.setHandlerName(handlerName);
        guaranty.setHandlerTelephone(handlerTelephone);
        guarantyService.saveGuaranty(guaranty);
        return ResultModel.ok();
    }
}
