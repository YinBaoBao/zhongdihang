...
public class UserController extends BaseController{
    ...
    @PreAuthorize("authenticated and hasPermission('user:delete')")
    ...
    public ResultModel addUser(@RequestParam("userName") String userName,@RequestParam("password") String password,
                               @RequestParam("realName") String realName, @RequestParam(value = "sex",required = false) String sex,
                               @RequestParam(value = "eMail",required = false) String eMail, @RequestParam("telephone") String telephone,
                               @RequestParam("userType") String userType, @RequestParam("orgId") Long orgId) {
        ...
        if (userType.equals(UserTypeEnum.BANK.getCode())){
            Bank bank=bankService.getBank(orgId);
            if (bank==null){
            ...
            Assess assess=assessService.getAssess(orgId);
            if (assess==null){
    ...
    public ResultModel editUser(@RequestParam("id") Long id, @RequestParam("userName") String userName,
                                @RequestParam("realName") String realName, @RequestParam(value = "sex",required = false) String sex,
                                @RequestParam(value = "eMail",required = false) String eMail, @RequestParam("telephone") String telephone,
                                @RequestParam("userType") String userType, @RequestParam("orgId") Long orgId) {
        User user = userService.getUser(id);
    ...
    public ResultModel modifyUserPassword(Long id, String oldPassword, String newPassword) {
        User user = userService.getUser(id);
        if (user == null) {
            return ResultModel.error(ResultStatus.FAILURE, "用户不存在");
        } else {
    ...
    public ResultModel roleAssign(@RequestParam  Long userId,@RequestParam String roleIds){
        if(userService.getUser(userId)==null){
            return ResultModel.error(ResultStatus.FAILURE,"用户不存在");
        }
        if(StringUtils.isEmpty(roleIds)){
            return ResultModel.error(ResultStatus.FAILURE,"请分配好权限");
        }
        String[] roleIdsStr=roleIds.split(",");
        Long[] roleIdLong=new Long[roleIdsStr.length];
        for (int i=0;i<roleIdsStr.length;i++){
            roleIdLong[i]= Long.parseLong(roleIdsStr[i]);
        }
        userService.userRole(userId,roleIdLong);
        return ResultModel.ok();
    }
}
